...
public class EpicSharkoRightClickedOnEntityProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if ((entity instanceof TamableAnimal _tamEnt ? _tamEnt.isTame() : false) && (entity instanceof TamableAnimal _tamEnt ? (Entity) _tamEnt.getOwner() : null) == sourceentity) {
			if (sourceentity.isShiftKeyDown()) {
				...
				if ((entity instanceof EpicSharkoEntity _datEntI ? _datEntI.getEntityData().get(EpicSharkoEntity.DATA_SharkoState) : 0) == 0) {
				...
				} else if ((entity instanceof EpicSharkoEntity _datEntI ? _datEntI.getEntityData().get(EpicSharkoEntity.DATA_SharkoState) : 0) >= 1
						&& (entity instanceof EpicSharkoEntity _datEntI ? _datEntI.getEntityData().get(EpicSharkoEntity.DATA_SharkoState) : 0) <= 3) {
					if (entity instanceof EpicSharkoEntity _datEntSetI)
						_datEntSetI.getEntityData().set(EpicSharkoEntity.DATA_SharkoState, 0);
					entity.getPersistentData().putDouble("SharkoStateTimerForChance", Math.round(Mth.nextDouble(RandomSource.create(), 30, 60)));
				}
			}
		}
