...
public class BiblicallyDeathSharkoRightClickedOnEntityProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if ((entity instanceof TamableAnimal _tamEnt ? _tamEnt.isTame() : false) && (entity instanceof TamableAnimal _tamEnt ? (Entity) _tamEnt.getOwner() : null) == sourceentity) {
			if (sourceentity.isShiftKeyDown()) {
				...
				if ((entity instanceof BiblicallyDeathSharkoEntity _datEntI ? _datEntI.getEntityData().get(BiblicallyDeathSharkoEntity.DATA_SharkoState) : 0) == 0) {
				...
				} else if ((entity instanceof BiblicallyDeathSharkoEntity _datEntI ? _datEntI.getEntityData().get(BiblicallyDeathSharkoEntity.DATA_SharkoState) : 0) >= 1
						&& (entity instanceof BiblicallyDeathSharkoEntity _datEntI ? _datEntI.getEntityData().get(BiblicallyDeathSharkoEntity.DATA_SharkoState) : 0) <= 3) {
					if (entity instanceof BiblicallyDeathSharkoEntity _datEntSetI)
						_datEntSetI.getEntityData().set(BiblicallyDeathSharkoEntity.DATA_SharkoState, 0);
					entity.getPersistentData().putDouble("SharkoStateTimerForChance", Math.round(Mth.nextDouble(RandomSource.create(), 30, 60)));
				}
			}
		}
