...
public class RareEngieSharkoRightClickedOnEntityProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if ((entity instanceof TamableAnimal _tamEnt ? _tamEnt.isTame() : false) && (entity instanceof TamableAnimal _tamEnt ? (Entity) _tamEnt.getOwner() : null) == sourceentity) {
			if (sourceentity.isShiftKeyDown()) {
				...
				if ((entity instanceof RareEngieSharkoEntity _datEntI ? _datEntI.getEntityData().get(RareEngieSharkoEntity.DATA_SharkoState) : 0) == 0) {
				...
				} else if ((entity instanceof RareEngieSharkoEntity _datEntI ? _datEntI.getEntityData().get(RareEngieSharkoEntity.DATA_SharkoState) : 0) >= 1
						&& (entity instanceof RareEngieSharkoEntity _datEntI ? _datEntI.getEntityData().get(RareEngieSharkoEntity.DATA_SharkoState) : 0) <= 3) {
					if (entity instanceof RareEngieSharkoEntity _datEntSetI)
						_datEntSetI.getEntityData().set(RareEngieSharkoEntity.DATA_SharkoState, 0);
					entity.getPersistentData().putDouble("SharkoStateTimerForChance", Math.round(Mth.nextDouble(RandomSource.create(), 30, 60)));
				}
			}
		}
