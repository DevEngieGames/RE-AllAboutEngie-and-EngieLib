...
public class EngieGamesMadEngieEntity extends Monster {
	public EngieGamesMadEngieEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EngiesChaosModEntities.ENGIE_GAMES_MAD_ENGIE.get(), world);
	}

	public EngieGamesMadEngieEntity(EntityType<EngieGamesMadEngieEntity> type, Level world) {
		super(type, world);
		maxUpStep = 1f;
		xpReward = 10;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(3, new NearestAttackableTargetGoal(this, Player.class, true, false) {
			...
			public boolean canUse() {
				double x = EngieGamesMadEngieEntity.this.getX();
				double y = EngieGamesMadEngieEntity.this.getY();
				double z = EngieGamesMadEngieEntity.this.getZ();
				Entity entity = EngieGamesMadEngieEntity.this;
				Level world = EngieGamesMadEngieEntity.this.level;
	...
	public static void init() {
		SpawnPlacements.register(EngiesChaosModEntities.ENGIE_GAMES_MAD_ENGIE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
