...
public class EngieGamesHostileEngieEntity extends Monster {
	public EngieGamesHostileEngieEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EngiesChaosModEntities.ENGIE_GAMES_HOSTILE_ENGIE.get(), world);
	}

	public EngieGamesHostileEngieEntity(EntityType<EngieGamesHostileEngieEntity> type, Level world) {
		super(type, world);
		maxUpStep = 1f;
		xpReward = 65;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(3, new NearestAttackableTargetGoal(this, Player.class, true, false) {
			...
			public boolean canUse() {
				double x = EngieGamesHostileEngieEntity.this.getX();
				double y = EngieGamesHostileEngieEntity.this.getY();
				double z = EngieGamesHostileEngieEntity.this.getZ();
				Entity entity = EngieGamesHostileEngieEntity.this;
				Level world = EngieGamesHostileEngieEntity.this.level;
	...
	public static void init() {
		SpawnPlacements.register(EngiesChaosModEntities.ENGIE_GAMES_HOSTILE_ENGIE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
